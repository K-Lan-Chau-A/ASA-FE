...
uniform float uTargetHue;        // 0..1
uniform float uHueReplaceStrength; // 0..1
...
void main(){
    ...
    // Force hue toward brand hue, scale by saturation to preserve contrast
    vec3 hsv=rgb2hsv(col.rgb);
    float hueMix = uHueReplaceStrength * clamp(hsv.y, 0.0, 1.0);
    hsv.x=mix(hsv.x,uTargetHue,hueMix);
    col.rgb=hsv2rgb(hsv);
    float scanline_val=sin(gl_FragCoord.y*uScanFreq)*0.5+0.5;
    col.rgb*=1.-(scanline_val*scanline_val)*uScan;
    col.rgb+=(rand(gl_FragCoord.xy+uTime)-0.5)*uNoise;
    // Subtle tint based on saturation to avoid flattening
    float tintMix = uTintStrength * (0.3 + 0.7 * hsv.y);
    col.rgb=mix(col.rgb,uTintColor,tintMix);
    // Gentle vignette to add depth
    vec2 uv = gl_FragCoord.xy/uResolution.xy*2.0-1.0;
    float vig = smoothstep(0.2, 1.0, dot(uv, uv));
    col.rgb *= (1.0 - uVignetteStrength * vig * 0.6);
...
export default function DarkVeil({
  ...
  warpAmount = 0,
  resolutionScale = 1,
  // Default tint to brand color #009DA5
  tintColor = [0 / 255, 157 / 255, 165 / 255],
  tintStrength = 0.15,
  // Replace hue toward brand hue (~#009DA5 -> ~0.51 in HSV)
  targetHue = 0.32,
  hueReplaceStrength = 0.6,
